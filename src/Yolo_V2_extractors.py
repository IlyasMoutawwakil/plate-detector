...
SQUEEZENET_BACKEND_PATH = BACKEND_DIR + \
    "squeezenet_backend.h5"  # should be hosted on the server
MOBILENET_BACKEND_PATH = BACKEND_DIR + \
    "mobilenet_backend.h5"   # should be hosted on the server
...
class FullYoloFeature(BaseFeatureExtractor):
    ...
    def __init__(self, input_size):
        # On peut choisir comme comme nombre de channel
...
class MobileNetFeature(BaseFeatureExtractor):
    ...
    def __init__(self, input_size):
        ...
        def conv_block(inputs, filters, width_multiplier, kernel_size=(3, 3), strides=(1, 1)):
            ...
            x = Conv2D(filters, kernel_size, padding='valid',
                       use_bias=False, strides=strides, name='conv1')(x)
        ...
        def mobile_block(inputs, pointwise_conv_filters, width_multiplier, depth_multiplier=1, strides=(1, 1), block_id=1):
            pointwise_conv_filters = int(
                pointwise_conv_filters*width_multiplier)
            ...
            x = DepthwiseConv2D((3, 3), padding='same' if strides == (
                1, 1) else 'valid', depth_multiplier=depth_multiplier, strides=strides, use_bias=False, name='conv_dw_%d' % block_id)(x)
        ...
        # Block2
        x = mobile_block(x, 128, width_multiplier,
                         depth_multiplier, strides=(2, 2), block_id=2)
        # Block 3
        x = mobile_block(x, 128, width_multiplier,
                         depth_multiplier, block_id=3)
        # Block 4
        x = mobile_block(x, 256, width_multiplier,
                         depth_multiplier, strides=(2, 2), block_id=4)
        # Block 5
        x = mobile_block(x, 256, width_multiplier,
                         depth_multiplier, block_id=5)
        # Block 6
        x = mobile_block(x, 512, width_multiplier,
                         depth_multiplier, strides=(2, 2), block_id=6)
        # Block 7
        x = mobile_block(x, 512, width_multiplier,
                         depth_multiplier, block_id=7)
        # Block 8
        x = mobile_block(x, 512, width_multiplier,
                         depth_multiplier, block_id=8)
        # Block 9
        x = mobile_block(x, 512, width_multiplier,
                         depth_multiplier, block_id=9)
        # Block 10
        x = mobile_block(x, 512, width_multiplier,
                         depth_multiplier, block_id=10)
        # Block 11
        x = mobile_block(x, 512, width_multiplier,
                         depth_multiplier, block_id=11)
        # Block 12
        x = mobile_block(x, 1024, width_multiplier,
                         depth_multiplier, strides=(2, 2), block_id=12)
        # Block 13
        x = mobile_block(x, 1024, width_multiplier,
                         depth_multiplier, block_id=13)
        ...
        self.feature_extractor = Model(
            inputs=input_image, outputs=x, name='mobilenet')
...
class SqueezeNetFeature(BaseFeatureExtractor):
    ...
    def __init__(self, input_size):
        ...
        self.feature_extractor = Model(
            inputs=input_image, outputs=x, name='squeezenet')
