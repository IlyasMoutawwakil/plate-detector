...
class YOLO(object):
    ...
    def __init__(self,
        ...
        # Visualisation de la forme de la sortie de l'extrateur de caractéristiques
        print(self.feature_extractor.get_output_shape())
        # Définition de la taille des grilles en fonction de la sortie de l'extracteur
        self.grid_h, self.grid_w = self.feature_extractor.get_output_shape()
        # Définir le Vecteur de caractéristiques
    ...
    def custom_loss(self, y_true, y_pred):
        ...
        # Ajustement de x et y : entre 0 et 1 plus offset
        pred_box_xy = tf.sigmoid(y_pred[..., :2]) + cell_grid
        pred_box_wh = tf.exp(y_pred[..., 2:4]) * np.reshape(self.anchors,
                                                            [1, 1, 1, self.nb_box, 2])  # Ajustement de w et h
        # Ajustement de la fidélité (Proba de présence)
        pred_box_conf = tf.sigmoid(y_pred[..., 4])
        # Ajustement des probabilités des classes
        ...
        conf_mask = conf_mask + tf.to_float(best_ious < 0.6) * (
            1 - y_true[..., 4]) * self.no_object_scale  # Celui qui ne doit pas parler
        conf_mask = conf_mask + y_true[..., 4] * \
            self.object_scale  # Celui qui doit savoir
    ...
    def train(self,
              ...
              # Le taux d'apprentissage initial : \ethat_{0}
              learning_rate,
              # La taille du batch : Puisqu'on utilise un entrainement par batch
              batch_size,
              warmup_epochs,               # Nombre d'époques pour l'échauffement
              # Pénalité sur les fausse négative : un objet n'est pas détecté
              object_scale,
              # Pénalité sur les fausse positive : une détection d'objets inexistant
              no_object_scale,
              coord_scale,                 # Pénalité sur les coordonnées du centre de l'objet
              class_scale,                 # Pénalité sur les dimensions de l'objet détecté
              saved_weights_dir=MODELS_DIR,  # Chemin d'enregistrement du model
        ...
        file_path = saved_weights_dir + \
            "EPOCH_{epoch:02d}_Validation_{val_loss:.2f}.h5"
    ...
    def predict(self, image, obj_threshold, nms_threshold):
        ...
        # Redimensionnement de l'image pour l'adapter à l'entrée du model
        image = cv2.resize(image, (self.input_size, self.input_size))
        # Normalisation de l'image d'entrée our l'extracteur
        image = self.feature_extractor.normalize(image)

        input_image = image[:, :, ::-1]  # Inversion des canaux de l'image
        # Ajouter un nouvelle dimension à l'image
        input_image = np.expand_dims(input_image, 0)
        # Le tenseur utilisé en entrainement
        dummy_array = np.zeros((1, 1, 1, 1, self.max_box_per_image, 4))

        netout = self.model.predict([input_image, dummy_array])[
            0]  # Prédiction du model
        # Décodage de la sortie du model
