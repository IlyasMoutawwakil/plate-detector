...
DATA_DIR = '/plate-detector/data/MOROCCAN_DATASET/'
...
def write_ann_file(img_name, boxes, object=OBJECT):
    ...
    data = ''
    for elm in boxes:
        top, left, bottom, right = min(boxes[elm][1], boxes[elm][3]), min(
            boxes[elm][0], boxes[elm][2]), max(boxes[elm][1], boxes[elm][3]), max(boxes[elm][0], boxes[elm][2])
        data += '{},{},{},{},{},{},{}\n'.format(
            object, top, left, bottom, right, height, width)
...
def update_coord(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        bbox_x = [x]
        bbox_y = [y]
        cv2.rectangle(img, (bbox_x[0], bbox_y[0]),
                      (bbox_x[-1], bbox_y[-1]), (0, 255, 0), 2, lineType=0)
    ...
    # L'évènement de bouger la souris tout en appuyan sur le bouton gauche
        ...
        for elm in boxes:
            cv2.rectangle(img, (boxes[elm][0], boxes[elm][1]),
                          (boxes[elm][2], boxes[elm][3]), (0, 255, 0), 2, lineType=0)
        bbox_x += [x]
        bbox_y += [y]
        cv2.rectangle(img, (bbox_x[0], bbox_y[0]),
                      (bbox_x[-1], bbox_y[-1]), (0, 255, 0), 2, lineType=0)
...
if __name__ == '__main__':
    ...
    for img_name in os.listdir(IMGS_DIR):
        ...
        # Mettre à jour la fonction update_coord avec les coordonnées et les évènement de la souris
        cv2.setMouseCallback(win_name, update_coord)
        ...
        # Boucle d'annotation
        while 1:
            # Visualisation de l'image mise à jour à chaque itération
            cv2.imshow(win_name, img)
            action = cv2.waitKey()
